...
use crate::chips::alu::cols::{AluCols, FieldOpEvent};
...
pub trait GenerateAluTrace<const R: usize> {
    ...
    fn build_trace<'a, F: Field, I: Iterator<Item = &'a FieldOpEvent<usize, F>>>(
        events: I,
        events_len: usize,
    ) -> RowMajorMatrix<F> {
...
macro_rules! operation_air {
    ($OpAir:ident, $op:tt, $events:ident) => {
        ...
        impl<const R: usize> $crate::chips::alu::air::Operation for $OpAir<R> {
            ...
            AB: p3_air::AirBuilder,
        {
            #[inline] fn eval(&self, builder: &mut AB) {
                <Self as $crate::chips::alu::air::AluAir<AB, R>>::eval(self, builder)
            }
        }

        // And your AluAir bridge for ANY builder AB
        impl<AB, const R: usize> $crate::chips::alu::air::AluAir<AB, R> for $OpAir<R>
        where
            AB: p3_air::AirBuilder,
        {}
...
mod test {
    ...
    use crate::chips::alu::air::{AddAir, AluAir, SubAir};
    ...
    {
        let trace = <A as crate::chips::alu::air::AluAir<
    ...
    fn do_test_bb_twoadic(log_blowup: usize, log_n: usize) -> Result<(), impl Debug> {
        use crate::chips::alu::air::MulAir;
