...
use p3_uni_stark::{
    CircuitBuilder, DummyCircuit, StarkConfig, SymbolicExpression, get_symbolic_constraints, prove,
...
fn test_symbolic_to_circuit() {
    ...
    let mut circuit = DummyCircuit::<Challenge>::new();
    let mut circuit_sels = [circuit.new_wire(), circuit.new_wire(), circuit.new_wire()];
    let mut circuit_public_values = [circuit.new_wire(), circuit.new_wire(), circuit.new_wire()];
    ...
    for _ in 0..NUM_FIBONACCI_COLS {
        circuit_local_values.push(circuit.new_wire());
        circuit_next_values.push(circuit.new_wire());
    }
    ...
    circuit.generate(
        ...
        next_values,
        &mut circuit_sels,
        &mut circuit_public_values,
        &mut circuit_local_values,
        &mut circuit_next_values,
    );

    assert!(folded_constraints == circuit.wires[sum.index].value.unwrap());
}
